...
class Exam1 extends Component {
  ...
  // 생성자 정의. props는 부모 컴포넌트로부터 전달 받은 데이터
  constructor(props) {
  ...
  // 컴포넌트가 처음 화면에 나타났을 때 수행됨
  componentDidMount() {
    console.log("componentDidMount : 마운트 완료");
  }

  // props나 state가 변경된 후 수행됨
  ...
  handleClick = () => {
    // 상태 업데이트. count 값을 기존값 +1 값으로 업데이트
  ...
  render() {
    return (
      <div>
        ...
        <h2>
          부모로부터 전달받은 값 : {this.props.br} {this.props.test}
        </h2>
      </div>
    );
  }
